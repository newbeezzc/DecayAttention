    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, mask) in enumerate(vali_loader):
                ...
                # decoder mask
                dec_mask = torch.ones([batch_y.shape[0], self.args.pred_len, batch_y.shape[-1]]).float()
                dec_mask = torch.cat([mask[:, :self.args.label_len, :], dec_mask], dim=1).float()

                # encoder - decoder
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, mask, dec_mask)[0]
                        else:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, mask, dec_mask)
                else:
                    if self.args.output_attention:
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, mask, dec_mask)[0]
                    else:
